...
export function UnitDetailsDialog({
  ...
  const isMarketToken =
    "priceUsd" in data &&
    "marketCapUsd" in data;

  const symbol = isMarketToken
    ? data.symbol
    : data.tokenSymbol || data.unitCode;
  const title = isMarketToken
    ? data.projectTitle
  ...
  const actions = [
    {
      ...
      onClick: () => {
        if (isMarketToken) {
          const returnTo =
            encodeURIComponent(
              `/exchange/${encodeURIComponent(data.symbol)}`
            );
          router.push(
    ...
    },
    {
      ...
      onClick: () => {
        ...
        if (targetSymbol) {
          router.push(
            `/exchange/${encodeURIComponent(targetSymbol)}`
          );
        }
      },
    },
  ...
  return (
    <UnitDetailsSheet
      ...
      features={
        <div
          className={`flex flex-col gap-1.5 ${featuresList.length > 2 ? "mt-1" : "mt-0"}`}
        >
          {featuresList
            .slice(0, 4)
            .map((f, i) => (
    ...
    >
      {isExpanded && (
        ...
        >
          ...
          <TabsContent
            value="plano"
            className="overflow-y-auto flex-1 mt-0 h-full pb-10"
          >
          ...
          <TabsContent
            value="caracteristicas"
            className="overflow-y-auto flex-1 mt-0 pb-10"
          >
            <div className="space-y-6">
              ...
              <section className="p-5 rounded-[28px] bg-primary/5 border border-primary/10">
                <h4 className="text-[10px] font-black uppercase tracking-widest text-primary mb-3 text-center">
                  Inversión Tokenizada
                </h4>
                <p className="text-[12px] text-primary/80 leading-relaxed text-center font-bold">
                  Esta unidad participa
                  del programa de renta
                  fija y apreciación de
                  mercado. Los tokens
                  son transferibles y
                  liquidables en tiempo
                  real.
                </p>
              </section>
            </div>
